...
const extractChecksum = (data) => {
  ...
  const etbIndex = Array.from(data).findIndex(function (byte) {
    return byte === 0x17;
  });
  const etxIndex = Array.from(data).findIndex(function (byte) {
    return byte === 0x03;
  });
...
const calculateChecksum = (data) => {
  ...
  const stxIndex = Array.from(data).findIndex(function (byte) {
    return byte === 0x02;
  });
  const etxIndex = Array.from(data).findIndex(function (byte) {
    return byte === 0x03;
