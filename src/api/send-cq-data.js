...
const sendToLabSpecAPI = async (jsonResult) => {
    try {
      ...
      const response = await fetch('https://lab-spec.systems/backend/biochemistry-analytics/with-equipment/without-mean-sd', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorMessage = 'Error sending to lab-spec API: ' + response.status + ' - ' + (response.statusText ||
          'An error occurred')
        log.error(errorMessage)
      }

      const result = await response.json()
      log.debug('Successfully sent data to lab-spec API:', JSON.stringify(result, null, 2))
      return result
