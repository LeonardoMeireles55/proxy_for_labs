...
const createLISConnection = (config, clientSocket) => {
  ...
  const scheduleReconnect = () => {
    ...
    cleanup()
    log.debug(`ForwardProxy -> Scheduling LIS reconnection in ${RECONNECT_INTERVAL/1000} seconds`)
...
const forwardToTarget = (data, targetSocket, clientSocket) => {
  ...
  try {
    log.debug(`ForwardProxy -> Received data from LIS ${targetSocket.remoteAddress}, forwarding to Client ${clientSocket.remoteAddress}`)
...
const handleClientConnection = (clientSocket) => {
  ...
  clientSocket.on('data', (data) => {
    log.debug(`ForwardProxy -> Received data from Client ${clientIP}, forwarding to LIS ${targetSocket.remoteAddress}`)
