...
const messageHandlers = {
  [ASTM.ENQ]: (equipment) => {
    log.info('Equipment client -> received ENQ, sending ACK')
    equipment.write(Buffer.from([ASTM.ACK]))
  },

  [ASTM.ACK]: () => {
    log.info('Equipment client -> received ACK, ready for data transmission')
  },

  [ASTM.EOT]: (equipment) => {
    log.info('Equipment client -> received EOT, closing connection')
    equipment.end()
  }
}
...
const handleData = (equipment, data) => {
  ...
  log.info('Equipment client -> echoing data back')
...
const createConnection = () => {
  const equipment = net.connect({
    ...
    log.info('Equipment client -> connected, sending ENQ to start communication')
  ...
  equipment.on('close', () => {
    log.info('Equipment client -> disconnected')
    scheduleReconnect()
  })
...
const scheduleReconnect = () => {
  setTimeout(() => {
    log.info('Equipment client -> reconnecting...')
